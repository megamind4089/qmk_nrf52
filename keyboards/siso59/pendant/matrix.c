...
static uint8_t matrix_row_read[(MATRIX_COLS+7)/8] = { 0xff };

// Address / Row config
#define IOEXPANDER_ADDRS  { 0x21, 0x21 }
#define DRIVER_ADDRS { &I2CD2, &I2CD1 };
...
static matrix_row_t read_cols(uint8_t row)
{
    ...
    status = i2cMasterReceiveTimeout(i2cDrivers[row], addresses[row],
                matrix_row_read, (MATRIX_COLS+7)/8, 1);
  ...
  return ((uint32_t)(matrix_row_read[0]))
      | ((uint32_t)(matrix_row_read[1]) << 8)
      | ((uint32_t)(matrix_row_read[2]) << 16)
      | ((uint32_t)(matrix_row_read[3]) << 24);
}
